...
# __scale__=10
__maxima__=10
__c__=rand(MersenneTwister(0),__maxima__)
__a__=5(rand(MersenneTwister(1),__maxima__,vars).-1/2)
function ff(x)
    __ret__=0
    for i ∈ 1:length(__c__)
        __ret__+=1/(__c__[i]+sum((x.-__a__[i,:]).^2))
    end
    return __ret__
    # __scale__*length(x) + sum(x.^2 - __scale__*cos.(2.0.*π.*x)) #Rastrigin
end


xx_train=5(rand(MersenneTwister(2),i_train,vars).-1/2)
# xx_train=10rand(i_train,vars)
yy_train=zeros(i_train)
for i=1:i_train yy_train[i]=ff(xx_train[i,:]) end
scatter3d(xx_train[:,1],xx_train[:,2],yy_train,label=false,
...
xx_test=5(rand(MersenneTwister(3),i_test,vars).-1/2)
# xx_test=10rand(i_test,vars)
yy_test=zeros(i_test)
for i=1:i_test yy_test[i]=ff(xx_test[i,:]) end
# yy_test.+=noise_fact.*rand(i_test)
