...
function ff(x)
    4x[1]^2-2.1x[1]^4+ (1/3)x[1]^6+x[1]*x[2]-4x[2]^2+4x[2]^4
end



xx_train=2*(rand(i_train,vars).-1/2)
i_keep=[]
for i ∈ 1:i_train
    if -sin(4π*xx_train[i,1])+2(sin(2π*xx_train[i,2]))^2<= 1.5
        push!(i_keep,i)
    end
end
i_keep=i_keep[1:10_000]
xx_train=xx_train[i_keep,:]
i_train=length(i_keep)


yy_train=zeros(i_train)
for i=1:i_train yy_train[i]=ff(xx_train[i,:]) end
scatter3d(xx_train[:,1],xx_train[:,2],yy_train,label=false,
    title="Gomez-Levy Function")
...
xx_test=2*(rand(i_test,vars).-1/2)
i_keep=[]
for i ∈ 1:i_test
    if -sin(4π*xx_test[i,1])+2(sin(2π*xx_test[i,2]))^2<= 1.5
        push!(i_keep,i)
    end
end
i_keep=i_keep[1:10_000]
xx_test=xx_test[i_keep,:]
i_test=length(i_keep)
